...
function requiredRepoDrilldownTables(useAggregates: boolean, canQueryAnalysis: boolean) {
  return [
    "gh_repo_metadata",
    "gh_repo_commits",
    "gh_repo_prs",
...
export async function repoDrilldown(repoName: string): Promise<RepoDrilldownPayload> {
  ...
  const request = requestReadiness(
    missing,
    requiredRepoDrilldownTables(useAggregates, canQueryAnalysis)
  );
  ...
  const feedQuery = useAggregates
    ? highWater.then((anchor) => q<RepoDrilldownFeedSqlRow>(
        `SELECT
