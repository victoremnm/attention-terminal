...
- For custom SQL, call listTables first, then describeTable for every referenced table, then run bounded read-only SQL with runReadOnlyQuery, then renderAnswer. Prefer one of the fixed payload types above over morphing-card whenever the question fits one of them; when none fit, call buildMorphingCard with runReadOnlyQuery's \`rows\` (unmodified) and a fitting \`visualizationType\`, then pass its returned object straight into renderAnswer.`;
