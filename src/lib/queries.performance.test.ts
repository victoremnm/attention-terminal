...
describe("query layer performance & structural optimization tests", () => {
  ...
  it("tickerLanes reads time-first trend tables anchored to their latest ingested bucket", async () => {
    await tickerLanes();
    ...
    expect(
      calls.some((sql) =>
        sql.includes("SELECT coalesce(max(hour), toStartOfHour(now())) FROM gh_repo_trend_hourly")
      )
    ).toBe(true);
    expect(
      calls.some((sql) =>
        sql.includes("SELECT coalesce(max(created_at), now()) FROM gh_repo_trend_feed")
      )
    ).toBe(true);
    expect(calls.some((sql) => sql.includes("GROUP BY repo_name, h"))).toBe(true);
    for (const sql of calls) {
      ...
      expect(sql).not.toContain("toStartOfHour(now()) - INTERVAL");
    }
  });
