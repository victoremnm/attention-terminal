  ...
  const sql = `
    ...
    FROM (
      SELECT
        ...
        uniqExactIf(bucket.actor_login, bucket.pushes > 0 AND coalesce(cls.actor_type, '') != 'Bot' AND (cls.actor_type != '' OR lower(bucket.actor_login) NOT LIKE '%[bot]%')) AS human_pusher_total,
