...
4. **Fail-Open Architecture**: If ClickHouse credentials are missing or network connections fail, telemetry helper scripts spool JSON payloads to `~/.claude/telemetry/spool.ndjson` for automated backfill.

## Decision Outcome
Accepted. All subagent runs invoke `./scripts/log-subagent-run.sh` and log session learnings to `session_learnings`. JSDoc schema summaries documented in `src/lib/telemetry-queries.ts`.
